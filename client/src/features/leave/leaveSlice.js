...
export const addLeaveApplication = createAsyncThunk(
  "leaveApplicationEmployee/addLeaveApplication",
  async (leaveApplication, thunkAPI) => {
    try {
...
export const getLeaveApplications = createAsyncThunk(
  "leaveApplicationEmployee/getAll",
  async (_, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
...
export const getAllLeaveApplications = createAsyncThunk(
  "leaveApplicationEmployee/getAllAdmin",
  async (_, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
...
export const updateLeaveApplicationStatus = createAsyncThunk(
  "leaveApplicationEmployee/updateStatus",
  async (leaveApplication, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await leaveService.updateLeaveApplicationStatus(
        leaveApplication,
        token
      );
    } catch (error) {
...
export const leaveSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(updateLeaveApplicationStatus.pending, (state) => {
      state.isLoading = true;
    });
    builder.addCase(updateLeaveApplicationStatus.fulfilled, (state, action) => {
      state.isLoading = false;
      state.isSuccess = true;
      state.leaveApplication = action.payload;
    });
